...
class MaterialJobController:
    ...
    def __init__(self,
        ...
        self.out_scf_dir=out_scf_dir
        fid.close()  # do not for get this when you finish using h5
        if not os.path.exists(self.out_scf_dir):
            os.makedirs(self.out_scf_dir)  # 重新创建空的output_dir文件夹
        ...
        self.scf_gpw_filename=os.path.join(out_scf_dir, self.scf_gpw_filename)
    ...
    def bands(self,nbands=16,kpath={'path':'GMKG', 'npoints': 60},convergece_band=8):
        self.calc_band = GPAW(self.scf_gpw_filename).fixed_density(
            nbands=nbands,
            kpts=kpath,
            symmetry='off',
            convergence={'bands':convergece_band}
        )
...
if __name__ == "__main__":
    print('running')
    controller = MaterialJobController('c2db-2022-11-30.h5', 6161, ecutoff_dft=400, ecutoff_gw=80)
    controller.scf()
    controller.bands(nbands=20,
                     kpath={'path':'GMKG', 'npoints': 60},
                     convergece_band=8)
    bd = controller.calc_band.band_structure()
    bd.plot(filename='bandstructure.png', show=True, emax=10.0)
